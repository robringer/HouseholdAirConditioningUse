# Household Air Conditioning Analysis, Part 3a
# Written By: Vineeth CR
...
my_k_fold_cv = function(X, y, k_folds = 5, folds_vec = NULL, verbose = FALSE, ...){
  ...
  for (k in 1 : k_folds){
    ...
    if (pred_type == "regression"){
      ...
      # Added by Renee
      NullModel_MAE_test = NullModel_MAE_test + sum(abs(test_data_k[, (p + 1)] - mean(y)))/n
      NullModel_RMSE_test = NullModel_RMSE_test + sqrt(sum((test_data_k[, (p + 1)] - mean(y))^2)/n)
    } else {
  ...
  if (pred_type == "regression"){
    list(y_hat = yhat_cv, L1_err = L1_err, L2_err = L2_err, rmse = sqrt(L2_err / n), PseudoRsq = 1 - L2_err / sum((y - mean(y))^2), folds = folds_vec,
         ...
         NullModel_MAE_test= NullModel_MAE_test, NullModel_RMSE_test = NullModel_RMSE_test # Added by Renee
         )
  } else {		
